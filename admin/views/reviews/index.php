...
$(document).ready(function () {
    ...
    function loadReviews() {
        $.getJSON("api.php?action=reviews", function (data)  {
            ...
            $.each(reviews, function (i, r) {
                ...
                html += '<td><button class="btn btn-danger btn-delete-review" data-id="'+r.idAvis+'">Delete</button></td>';
                html += '</tr>';
            });
    ...
    // ── Supprimer un avis via fetch POST ──────────────────────────────────────
        ...
        $.post("reviews.php?action=delete_review", { id: id }, function (res) {
            if (res.success) {
                $row.fadeOut(300, function () {
                    ...
                    var count = parseInt($("#reviewCount").text()) - 1;
                    $("#reviewCount").text(count);
                    // recharger les stats (moyenne, 5 étoiles changent)
                    loadReviews();
                });
